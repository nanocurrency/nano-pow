		...
		else
		{
			...
			if (driver != nullptr && result)
			{
				...
				else if (operation == "profile")
				{
					...
					std::cout << "Profiling threads: " << std::to_string (threads_l) << " lookup: " << std::to_string (lookup_entries / (1024 * 1024) * sizeof (uint32_t)) << "MB threshold: " << to_string_hex128 (threshold) << " difficulty: " << to_string_hex128 (driver_difficulty) << " (" << to_string_hex64 (nano_pow::difficulty_128_to_64 (driver_difficulty)) << ")" << std::endl;
