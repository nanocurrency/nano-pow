...
Source file was modified for Nano PoW
...
public:
	...
	{
		uint128 t;
		t.as_longlongs[0] = ~as_longlongs[0];
		t.as_longlongs[1] = ~as_longlongs[1];
		return t;
	}
...
// Looks like this fails with SIGFPE on both GCC and clang no matter what
	...
	// uint128 operator^(uint128 v) const noexcept;
	// uint128 operator^=(uint128 v) noexcept;
	uint128 operator| (uint64_t v) const noexcept
	{
		uint128 t (*this);
		t.as_longlongs[0] |= v;
		return t;
	...
	bool operator< (const uint128 & o) const noexcept
	{
		return as_longlongs[1] < o.as_longlongs[1] || (as_longlongs[1] == o.as_longlongs[1] && as_longlongs[0] < o.as_longlongs[0]);
	}
	bool operator<= (const uint128 & o) const noexcept
	{
		return as_longlongs[1] < o.as_longlongs[1] || (as_longlongs[1] == o.as_longlongs[1] && as_longlongs[0] <= o.as_longlongs[0]);
...
namespace nano_pow
{
using uint128_t = integers128::uint128;
}
